...
export const InquiryModal: React.FC<InquiryModalProps> = ({
  ...
  // Reset or initialize lines when modal opens or preselected jacket changes
  useEffect(() => {
    if (isOpen) {
        ...
        {!submitted ? (
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <form onSubmit={handleSubmitOrder} className="flex-1 overflow-y-auto pr-1 space-y-6">
              {/* Type Selection - Direct Order & Workshop Appointment */}
              ...
              {/* ------------------------------------------------------------- */}
              {/* DYNAMIC ORDER LINES (3-COLUMN TABLE WITH '+' BUTTON)          */}
              {/* ------------------------------------------------------------- */}
              <div className="p-4 sm:p-5 rounded-2xl bg-[#121713] border border-[#344437] space-y-4">
                ...
                <div className="space-y-3.5">
                  {orderLines.map((line, index) => {
                    ...
                    return (
                      ...
                      >
                        {/* 1. NOM DE L'ARTICLE CLAIREMENT AU-DESSUS DES MENUS DÉROULANTS */}
                        <div className="flex items-center justify-between border-b border-[#273429] pb-2.5">
                          <div className="flex items-center space-x-2.5">
                            ...
                            <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#273429] text-[#b89f74] border border-[#3c4e40] hidden sm:inline font-sans">
                              {currentJacket?.category || 'Pièce Signature'}
                            </span>
                          </div>

                          <div className="flex items-center space-x-3">
                            <span className="text-xs font-semibold text-[#d4af37]">
                              {currentJacket?.price} {currentJacket?.currency || primaryCurrency} <span className="text-[10px] text-[#8e9f90] font-normal">/ unité</span>
                            </span>
                            {orderLines.length > 1 && (
                              <button
                                type="button"
                        ...
                        {/* 2. LES 3 COLONNES DE MENUS DÉROULANTS PARFAITEMENT ALIGNÉS */}
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                          {/* COLONNE 1: CHOIX DU MODÈLE */}
                          <div className="md:col-span-5 flex items-center space-x-3">
                            <div className="w-11 h-11 rounded-xl bg-[#0e130f] border border-[#38483b] overflow-hidden flex-shrink-0 relative shadow-inner">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <select
                                ...
                                className="w-full h-10 bg-[#121613] border border-[#3d4f40] text-xs text-[#f3ece0] rounded-xl px-3 py-2 focus:border-[#d4af37] outline-none font-medium truncate cursor-pointer transition-colors"
                              >
                                {validJackets.map((j) => (
                          ...
                          {/* COLONNE 2: NUANCE ET TAILLE (ALIGNÉS) */}
                          <div className="md:col-span-4 grid grid-cols-2 gap-2 items-center">
                            <div className="min-w-0">
                              ...
                              <select
                                ...
                                className="w-full h-10 bg-[#121613] border border-[#3d4f40] text-xs text-[#f3ece0] rounded-xl px-2.5 py-2 focus:border-[#d4af37] outline-none cursor-pointer truncate transition-colors"
                              >
                            ...
                            <div className="min-w-0">
                              ...
                              <select
                                ...
                                className="w-full h-10 bg-[#121613] border border-[#3d4f40] text-xs text-[#f3ece0] rounded-xl px-2.5 py-2 focus:border-[#d4af37] outline-none font-bold cursor-pointer transition-colors"
                              >
                          ...
                          {/* COLONNE 3: QUANTITÉ ET SOUS-TOTAL */}
                            ...
                            <div className="flex items-center justify-between space-x-2">
                              ...
                              <div className="h-10 flex items-center space-x-1 bg-[#121613] border border-[#3d4f40] rounded-xl px-1.5 py-1">
                                <button
                                  ...
                                  className="w-7 h-7 rounded-lg flex items-center justify-center text-xs text-[#a3b1a5] hover:text-white hover:bg-[#253228] disabled:opacity-30 cursor-pointer font-bold transition-colors"
                                >
                                  -
                                </button>
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleUpdateLineField(line.id, 'quantity', line.quantity + 1)}
                                  className="w-7 h-7 rounded-lg flex items-center justify-center text-xs text-[#a3b1a5] hover:text-white hover:bg-[#253228] cursor-pointer font-bold transition-colors"
                                >
                                  +
                                </button>
                              </div>
                              ...
                              <div className="text-right min-w-[70px]">
                                <span className="font-serif text-sm font-bold text-[#d4af37] block leading-tight">
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-[#29362b]">
                  <button
                    ...
                    className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-[#232f25] border border-[#d4af37]/70 text-[#d4af37] hover:bg-[#2d3d30] text-xs font-semibold uppercase tracking-wider flex items-center justify-center space-x-2 transition-all shadow-md active:scale-95 cursor-pointer"
                  >
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-[#b89f74] via-[#d4af37] to-[#8c6d3f] text-[#121613] font-serif font-bold text-sm uppercase tracking-widest hover:brightness-110 active:scale-[0.99] transition-all shadow-xl flex items-center justify-center space-x-2 cursor-pointer"
                >
                ...
                <p className="text-[11px] text-[#7d8d7f] text-center mt-2">
                  Paiement sécurisé et validation personnalisée avec notre maître tailleur par email et téléphone.
                </p>
              </div>
            </form>
          </div>
          ...
          /* ============================================================= */
          /* RECAPITULATION & CONFIRMATION SCREEN                           */
          /* ============================================================= */
          <div className="py-6 space-y-5 animate-fadeIn max-h-[85vh] overflow-y-auto pr-1">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-[#a3b1a5] max-w-md mx-auto">
                Votre commande a été enregistrée avec succès et transmise à notre atelier pyrénéen (<strong className="text-[#d4af37]">{ordersEmail}</strong>).
              </p>
            </div>

            {/* Ordered Items Detailed Summary Table */}
            ...
            <div className="p-3.5 rounded-xl bg-[#18231b] border border-[#3b4e3e] text-xs text-[#b8c5ba] space-y-1">
              <p>
                Un récapitulatif détaillé vous a été adressé à <strong>{clientEmail}</strong>. Notre maître tailleur vérifiera vos options sous 24h ouvrées.
              </p>
              {clientNotes && (
                <p className="text-[11px] text-[#a3b1a5] italic pt-1 border-t border-[#29372c]">
                  Remarques transmises : "{clientNotes}"
                </p>
              )}
            </div>
