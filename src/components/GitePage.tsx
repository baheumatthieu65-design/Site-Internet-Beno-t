...
export const GitePage: React.FC = () => (
  <main className="gite-page">
    ...
    <section className="gite-section gite-intro">
      ...
      <h2>{c.intro.title}</h2><p>{c.intro.text}</p>
    </section>
    <section className="gite-section">
      <p className="gite-eyebrow">Le gîte en images</p>
      <div className="gite-gallery">
        {c.gallery.map(i => <img key={i.src} src={i.src} alt={i.alt} />)}
      </div>
    </section>
    {c.videoUrl && <section className="gite-video">
      <video controls playsInline preload="metadata" poster={c.videoPoster}><source src={c.videoUrl} /></video>
    </section>}
    <section className="gite-section">
      <p className="gite-eyebrow">Les essentiels</p>
      <div className="gite-essentials">
        {c.essentials.map(i => <div className="gite-card" key={i.label}><strong>{i.value}</strong><span>{i.label}</span></div>)}
      </div>
    </section>
