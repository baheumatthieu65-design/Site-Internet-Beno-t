...
export const GiteCustomizerPanel:React.FC<Props>=({value,onChange})=>{
  ...
  return <div className="space-y-6">
    <div className={panel}><div><h4 className="font-serif text-lg text-[#f3ece0]">Page Gîte — contenus</h4><p className="text-xs text-[#a3b1a5]">Cette page possède sa propre configuration : les modifications ne touchent pas la Boutique.</p></div>
      <div className="grid md:grid-cols-2 gap-4">
        {([['name','Nom du gîte'],['location','Lieu / région'],['tagline','Accroche'] ] as const).map(([k,l])=><label key={k} className="text-xs text-[#a3b1a5]">{l}<input value={(c as any)[k]||''} onChange={e=>update({[k]:e.target.value} as any)} className="mt-2 w-full bg-[#101510] border border-[#344237] rounded-xl px-3 py-2 text-white"/></label>)}
      </div>
