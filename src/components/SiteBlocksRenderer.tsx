    ...
    >
      {blocks.map((block) => {
        ...
        if (block.type === 'image' && block.url) {
          return <img key={block.id} src={block.url} alt="" className="max-w-[280px] max-h-[220px] rounded-xl object-cover shadow-2xl" style={common} />;
        }

        if (block.type === 'video' && block.url) {
          return <video key={block.id} src={block.url} autoPlay muted loop playsInline className="max-w-[420px] max-h-[260px] rounded-xl object-cover shadow-2xl" style={common} />;
        }
        ...
        if (block.type === 'heading') {
          return <div key={block.id} className="px-3 py-2 text-2xl sm:text-4xl font-serif font-semibold text-[#f5eedf] drop-shadow-xl whitespace-nowrap" style={common}>{block.text || 'Nouveau titre'}</div>;
        }
