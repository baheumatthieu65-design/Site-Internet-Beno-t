...
export const AdminBar: React.FC<AdminBarProps> = ({ username, theme, isDragReorderMode=true, onToggleDragReorderMode, onOpenEditor, onOpenOrders, onOpenProducts, onOpenLogoEditor, onToggleFloatingMedia, floatingMediaOpen=false, onOpenSecurity, onLogout, embedded=false }) => {
  ...
  return <div id="admin-top-bar" className={embedded ? 'rounded-xl border border-[#d4af37]/40 bg-[#161c17] px-3 py-3 text-xs text-[#f3ece0] shadow-inner' : 'fixed bottom-0 left-0 right-0 z-50 border-t border-[#d4af37]/50 bg-[#161c17] px-4 py-2 text-xs text-[#f3ece0] shadow-2xl backdrop-blur-md'}>
    <div className={embedded ? 'flex flex-wrap items-center justify-between gap-3' : 'mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3'}>
      ...
      <div className="flex w-full flex-col items-end gap-2 lg:w-auto"><div className="flex flex-wrap justify-end gap-1.5 sm:gap-2">
        ...
        {onToggleFloatingMedia && <button onClick={onToggleFloatingMedia} className={`flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer border ${floatingMediaOpen ? 'bg-[#d4af37] text-[#121613] border-[#d4af37]' : 'bg-[#202922] hover:bg-[#2c382e] text-[#c4ceb8] border-[#3b4b3e] hover:border-[#d4af37]'}`}><ImageIcon className="w-3.5 h-3.5"/><span>Image flottante</span></button>}
