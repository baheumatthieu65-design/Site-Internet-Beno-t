          ...
          <div className="grid grid-cols-[190px_1fr] gap-3">
            ...
            {selected ? <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                <label className="text-[11px] text-[#a3b1a5]">Fond<div className="mt-1 flex items-center gap-2"><input type="color" value={/^#[0-9a-fA-F]{6}$/.test(selected.backgroundColor || '') ? selected.backgroundColor : '#ffffff'} disabled={selected.backgroundColor === 'transparent'} onChange={e=>updateBlock(selected.id,{backgroundColor:e.target.value})} className="h-9 w-12 rounded bg-transparent cursor-pointer disabled:opacity-40"/><input disabled={selected.backgroundColor === 'transparent'} value={selected.backgroundColor === 'transparent' ? 'Transparent' : (selected.backgroundColor || '')} onChange={e=>updateBlock(selected.id,{backgroundColor:e.target.value})} placeholder="#ffffff ou rgba(...)" className="min-w-0 flex-1 rounded-lg bg-[#18201a] border border-[#344237] px-2 py-2 text-white"/></div></label>
