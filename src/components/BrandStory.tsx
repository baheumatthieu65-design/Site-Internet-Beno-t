      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-sm sm:text-base text-[#b8c5ba] leading-relaxed">
            <div className={`p-8 rounded-3xl ${cardStyle.card} space-y-6`}><Quote className="w-10 h-10 text-[#d4af37]/30" />{brandData.storyText1 && <p data-vce-id="story-text-1" className="font-serif text-lg text-[#f3ece0] italic">{brandData.storyText1}</p>}{brandData.storyText2 && <p data-vce-id="story-text-2">{brandData.storyText2}</p>}<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-[#2e3b30] text-center"><div className="p-3 rounded-2xl bg-[#1d251f]"><span className="font-serif text-2xl font-bold text-[#d4af37] block">100%</span><span data-vce-id="story-stat-label-local-wool" className="text-[11px] uppercase text-[#a3b0a2] tracking-wider">Laine Vierge Locale</span></div><div className="p-3 rounded-2xl bg-[#1d251f]"><span className="font-serif text-2xl font-bold text-[#d4af37] block">2</span><span data-vce-id="story-stat-label-models" className="text-[11px] uppercase text-[#a3b0a2] tracking-wider">Modèles d'Exception</span></div><div className="p-3 rounded-2xl bg-[#1d251f]"><span className="font-serif text-2xl font-bold text-[#d4af37] block">Pyrénées</span><span data-vce-id="story-stat-label-mountain" className="text-[11px] uppercase text-[#a3b0a2] tracking-wider">Conception de Montagne</span></div></div></div>
          </div>
        </div>
      </div>
    </section>
