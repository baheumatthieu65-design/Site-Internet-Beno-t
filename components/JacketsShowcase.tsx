...
import { CreationType, JacketModel, Hotspot, ThemeConfig, ProductBlockId } from '../types';
import { CreationTypeTabs } from './CreationTypeTabs';
import { getCreationTypes, getProductCreationTypeId, getCreationTypeLabel } from '../utils/creationTypes';
import {
  Check,
  Info,
  Shield,
...
interface JacketsShowcaseProps {
  ...
  selectedCreationTypeId: string;
  creationTypes: CreationType[];
  onSelectCreationType: (typeId: string) => void;
...
export const JacketsShowcase: React.FC<JacketsShowcaseProps> = ({
  ...
  const availableCreationTypes = getCreationTypes(creationTypes);
  const selectedType = availableCreationTypes.find((type) => type.id === selectedCreationTypeId) || availableCreationTypes[0];
  const selectedTypeId = selectedType?.id || 'signature';
  const counts = availableCreationTypes.reduce<Record<string, number>>((acc, type) => {
    acc[type.id] = (Array.isArray(jackets) ? jackets : []).filter((product) => getProductCreationTypeId(product) === type.id).length;
    return acc;
  }, {});

  const orderedJackets = sortProductsByAvailability(Array.isArray(jackets) ? jackets : [])
    .filter((product) => getProductCreationTypeId(product) === selectedTypeId);
      ...
      <div className={`${containerWidthClass} px-4 sm:px-6 lg:px-8 relative z-10`}>
        ...
        <div className={`${textAlignClass} max-w-3xl mx-auto mb-10`}>
          ...
          <h2 className="font-serif text-3xl sm:text-5xl font-light text-[#f3ece0] mt-2 mb-4">
            Nos {visibleJackets.length} {visibleJackets.length > 1 ? 'Créations' : 'Création'} {getCreationTypeLabel(availableCreationTypes, selectedTypeId)}
          </h2>
