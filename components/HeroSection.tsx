...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const creationTypes = getCreationTypes(brandData.creationTypes);
  const selectedType = creationTypes.find((type) => type.id === selectedCreationTypeId) || creationTypes[0];
  const counts = creationTypes.reduce<Record<string, number>>((acc, type) => {
    acc[type.id] = allJackets.filter((product) => getProductCreationTypeId(product) === type.id).length;
    return acc;
  }, {});
  ...
  const jackets = sortProductsByAvailability(allJackets)
    .filter((j) => getProductCreationTypeId(j) === selectedTypeId)
        ...
        {layout === 'centered-minimal' ? (
          <div className={`${textAlignClass} max-w-4xl mx-auto space-y-6`}>
            ...
            <div className={`pt-6 flex flex-wrap items-center ${buttonAlignClass} gap-4`}>
              ...
              >
                <span data-vce-id="hero-discover-button-text">{discoverText} la collection ({jackets.length} {jackets.length > 1 ? 'créations' : 'création'} {getCreationTypeLabel(creationTypes, selectedTypeId)})</span>
