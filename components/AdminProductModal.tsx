...
import { ComparisonCriterion, CreationType, JacketModel, JacketAvailabilityStatus } from '../types';
...
export const AdminProductModal: React.FC<AdminProductModalProps> = ({
  ...
  const handleStartCreate = () => {
    ...
    setEditingProduct({
      ...
      creationTypeId: getCreationTypes(creationTypes)[0]?.id || 'signature',
      price: 490,
      currency: '€',
          ...
          {editingProduct ? (
            ...
            <form onSubmit={handleSaveProduct} className="space-y-5 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  <label className="block text-[#a3b1a5] font-semibold mb-1">Typologie de création</label>
                  <select
                    value={getProductCreationTypeId(editingProduct as JacketModel)}
                    onChange={(e) => updateEditingProduct({ creationTypeId: e.target.value })}
                    className="w-full bg-[#121613] border border-[#38483b] text-white px-3.5 py-2.5 rounded-xl outline-none focus:border-[#d4af37] cursor-pointer"
                  >
                    {getCreationTypes(creationTypes).map((type) => (
                      <option key={type.id} value={type.id}>{type.label}</option>
                    ))}
                  </select>
                  <p className="text-[10px] text-[#718073] mt-1.5">Cette typologie détermine dans quel onglet l’article apparaît sur la boutique, le Hero et le comparatif. Elle ne modifie pas la catégorie ni les réglages de la fiche article.</p>
                </div>
