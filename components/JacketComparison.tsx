...
import { CreationType, JacketModel, ThemeConfig, ComparisonCriterion } from '../types';
...
export const JacketComparison: React.FC<JacketComparisonProps> = ({
  ...
  const counts = availableCreationTypes.reduce<Record<string, number>>((acc, type) => {
    acc[type.id] = jackets.filter((product) => getProductCreationTypeId(product) === type.id).length;
    return acc;
  }, {});
      ...
      <div className={`${containerWidthClass} px-4 sm:px-6 lg:px-8`}>
        ...
        <div className={`${textAlignClass} max-w-3xl mx-auto mb-16`}>
          ...
          <h2 data-vce-id="comparison-title" className="font-serif text-3xl sm:text-5xl font-light text-[#f3ece0]">
            {sectionTitle} — {getCreationTypeLabel(availableCreationTypes, selectedTypeId)}
          </h2>
