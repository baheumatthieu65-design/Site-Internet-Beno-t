...
import { DEFAULT_CREATION_TYPES, getCreationTypes, getProductCreationTypeId } from '../utils/creationTypes';
...
export const BrandCustomizerModal: React.FC<BrandCustomizerModalProps> = ({
  ...
  const [formData, setFormData] = useState<BrandConfig>(() => {
    ...
    if (!Array.isArray(copy.creationTypes) || copy.creationTypes.length === 0) {
      copy.creationTypes = JSON.parse(JSON.stringify(DEFAULT_CREATION_TYPES));
    }
    if (!copy.ordersEmail) {
  ...
  const activeCreationTypes = getCreationTypes(formData.creationTypes);

  const handleAddCreationType = (label: string) => {
    const cleanLabel = label.trim();
    if (!cleanLabel) return;
    if (activeCreationTypes.some((type) => type.label.toLocaleLowerCase() === cleanLabel.toLocaleLowerCase())) {
      alert('Cette typologie existe déjà.');
      return;
    }
    const newType: CreationType = {
      id: `creation-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      label: cleanLabel,
    };
    setFormData((prev) => ({ ...prev, creationTypes: [...activeCreationTypes, newType] }));
    setNewCreationTypeLabel('');
  };

  const handleRenameCreationType = (id: string, label: string) => {
    const cleanLabel = label.trim();
    if (!cleanLabel) return;
    setFormData((prev) => ({
      ...prev,
      creationTypes: getCreationTypes(prev.creationTypes).map((type) => type.id === id ? { ...type, label: cleanLabel } : type),
    }));
  };

  const handleDeleteCreationType = (id: string) => {
    if (activeCreationTypes.length <= 1) {
      alert('Le catalogue doit conserver au moins une typologie de création.');
      return;
    }
    const target = activeCreationTypes.find((type) => type.id === id);
    if (!target) return;
    if (!window.confirm(`Supprimer la typologie « ${target.label} » ? Les articles concernés seront basculés vers la première typologie restante.`)) return;

    const remaining = activeCreationTypes.filter((type) => type.id !== id);
    const fallbackId = remaining[0].id;
    setFormData((prev) => ({ ...prev, creationTypes: remaining }));
    setDraftProducts((current) => current.map((product) =>
      getProductCreationTypeId(product) === id ? { ...product, creationTypeId: fallbackId } : product
    ));
  };
  ...
  const [newCreationTypeLabel, setNewCreationTypeLabel] = useState('');
            ...
            <div className="animate-fadeIn min-h-full">
              <div className="mb-6 p-5 rounded-2xl bg-[#1a221c] border border-[#3c4c3f] space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 text-[#d4af37]">
                      ...
                      <h4 className="font-serif text-base font-bold text-[#f3ece0]">Typologies de créations</h4>
                    </div>
                    <p className="text-xs text-[#a3b1a5] mt-1">Créez les onglets de votre boutique puis affectez chaque article à l’une de ces typologies. Les réglages existants de vos articles restent inchangés.</p>
                  </div>
                  <span className="text-xs text-[#a3b1a5] whitespace-nowrap">{activeCreationTypes.length} typologie(s)</span>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {activeCreationTypes.map((type) => {
                    const count = draftProducts.filter((product) => getProductCreationTypeId(product) === type.id).length;
                    return (
                      <div key={type.id} className="flex items-center gap-2 p-3 rounded-xl bg-[#121613] border border-[#2e3b30]">
                        <input
                          value={type.label}
                          onChange={(e) => handleRenameCreationType(type.id, e.target.value)}
                          className="min-w-0 flex-1 bg-transparent border-b border-[#38483b] focus:border-[#d4af37] text-sm text-[#f3ece0] px-1 py-1 outline-none"
                        />
                        <span className="text-[10px] text-[#7f8e80] whitespace-nowrap">{count} article{count > 1 ? 's' : ''}</span>
                        <button type="button" onClick={() => handleDeleteCreationType(type.id)} className="p-1.5 rounded-lg text-red-300 hover:bg-red-950/40 cursor-pointer" title="Supprimer cette typologie">
                ...
                <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-[#2a362c]">
                  <input
                    value={newCreationTypeLabel}
                    onChange={(e) => setNewCreationTypeLabel(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddCreationType(newCreationTypeLabel); } }}
                    placeholder="Nouvelle typologie (ex. Sport, Goodies, Accessoires...)"
                  ...
                  />
                  <button type="button" onClick={() => handleAddCreationType(newCreationTypeLabel)} className="px-4 py-2.5 rounded-xl bg-[#28362b] border border-[#d4af37] text-[#d4af37] text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 cursor-pointer">
                    <Plus className="w-4 h-4" /> Ajouter une typologie
                  </button>
                </div>
              </div>
              ...
              <AdminProductModal
                ...
                creationTypes={activeCreationTypes}
