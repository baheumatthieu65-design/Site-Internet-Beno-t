...
import { getCreationTypes, getProductCreationTypeId } from './utils/creationTypes';
...
export default function App() {
  ...
  const [selectedCreationTypeId, setSelectedCreationTypeId] = useState<string>(() =>
    getCreationTypes(brandData.creationTypes)[0]?.id || 'signature'
  );

  const handleSelectCreationType = (typeId: string) => {
    const types = getCreationTypes(brandData.creationTypes);
    const validTypeId = types.some((type) => type.id === typeId) ? typeId : types[0]?.id || 'signature';
    setSelectedCreationTypeId(validTypeId);

    const productsForType = (brandData.jackets || []).filter(
      (product) => getProductCreationTypeId(product) === validTypeId
    );
    const nextProductId = getDefaultShowcaseProductId(productsForType);
    if (nextProductId) setSelectedJacketId(nextProductId);
  };

  useEffect(() => {
    const types = getCreationTypes(brandData.creationTypes);
    const currentType = types.find((type) => type.id === selectedCreationTypeId) || types[0];
    const validTypeId = currentType?.id || 'signature';
    if (validTypeId !== selectedCreationTypeId) {
      setSelectedCreationTypeId(validTypeId);
    }
    ...
    if (!productsForType.some((product) => product.id === selectedJacketId)) {
      const nextProductId = getDefaultShowcaseProductId(productsForType);
      if (nextProductId) setSelectedJacketId(nextProductId);
    }
  }, [brandData.creationTypes, brandData.jackets, selectedCreationTypeId, selectedJacketId]);
      ...
      case 'hero':
        content = (
          <HeroSection
            ...
            selectedCreationTypeId={selectedCreationTypeId}
      ...
      case 'collection':
        content = (
          <JacketsShowcase
            ...
            selectedCreationTypeId={selectedCreationTypeId}
            creationTypes={getCreationTypes(brandData.creationTypes)}
            onSelectCreationType={handleSelectCreationType}
            onOpenInquiry={
              handleOpenInquiry
            }
